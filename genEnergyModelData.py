...
# E_b - battery energy
# E_r - regenerative braking energy
# K_m - efficiency factor for battery energy conversion to kinetic energy efficiency
# K_r - efficiency factor for regenerative braking energy conversion to electrical energy
# E_h - battery energy loss due to heat
...
class VehicleParams:
    ...
    mass: float = 1800.0              # Vehicle mass (kg)
    ...
    K_r: float = 0.50                  # Regenerative braking efficiency
    aux_power: float = 0.5             # Auxiliary systems power consumption (kW)
...
def calculate_rolling_resistance(
    ...
    return vehicle.rolling_resistance * vehicle.mass * env.gravity * np.cos(theta)


def calculate_grade_force(
    vehicle: VehicleParams, 
    env: EnvironmentParams, 
    grade: float
) -> float:
    ...
    return vehicle.mass * env.gravity * np.sin(theta)


def calculate_acceleration_force(
    vehicle: VehicleParams, 
    acceleration: float
) -> float:
    """
    ...
    F_accel = m * a
    """
    return vehicle.mass * acceleration
...
def calculate_total_force(
    ...
    """Calculate total force required to move the vehicle"""
    F_drag = calculate_aerodynamic_drag(velocity, vehicle, env)
    F_roll = calculate_rolling_resistance(vehicle, env, grade)
    F_grade = calculate_grade_force(vehicle, env, grade)
    F_accel = calculate_acceleration_force(vehicle, acceleration)
    
    return F_drag + F_roll + F_grade + F_accel
...
def main():
    ...
    df1 = generate_scenario_data(duration=36000, speed_kmh=80, initial_soc=95)
